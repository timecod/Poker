...
Game::Game (Player ** players, int n, int diler) {
	this->diler = CircleInt(diler, n);
	who_walk = CircleInt(diler + 1, n);
	step = 0;
	last = 0;
...
void Game::update () {
	...
	if (step == 0 && who_walk - diler == 1) { players[who_walk]->change_coins(-50); }
		...
		if (players[who_walk]->is_playing()) {
			int equals = count = 1;
			int base = players[who_walk]->get_coins_in_game();
			for (CircleInt i(who_walk + 1, players.size()); i - who_walk != 0; ++i) {
				int t = players[i]->is_playing();
				count += t;
				equals += t * players[i]->get_coins_in_game() == base;
			}

			if (step > 0 && equals == count) {
				step++;
...
int str_to_int (std::string str) {
	...
	for (auto i = str.begin(); i < str.end(); ++i) {
		...
		if (!flag) throw "Can't read. Input numbers";
	}
	return r;
}
...
void Game::show () {
	...
	std::system("clear");
	std::vector <std::vector <S>> strings(5);
	for (int i = 0; i < players.size(); ++i) {
		S is_walk = i == diler ? "D" : i == who_walk ? "@" : " ";
		S str_name = players[i]->get_name();
		S str_status = players[i]->is_playing() ? "p" : "n";
	...
	for (int i = 0; i < players.size(); ++i) {
		str_out[i] = strings[4][i] + " -- " + strings[0][i] + " (" + strings[3][i] + "): " + strings[1][i] + " | " + strings[2][i];
	}
